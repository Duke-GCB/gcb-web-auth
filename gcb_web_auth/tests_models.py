...
class DDSUserCredentialTestCase(TestCase):
    ...
    def setUp(self):
        self.endpoint = DDSEndpoint.objects.create(name='endpoint1',
    ...
    def test_creates_with_user_and_endpoint(self):
        DDSUserCredential.objects.create(user=self.user1, endpoint=self.endpoint)

    def test_requires_user_relationship(self):
        with self.assertRaises(IntegrityError) as e:
            DDSUserCredential.objects.create(user=None, endpoint=self.endpoint)
    ...
    def test_unique_token(self):
        DDSUserCredential.objects.create(user=self.user1, endpoint=self.endpoint, token='token1')
        with self.assertRaises(IntegrityError):
            DDSUserCredential.objects.create(user=self.user1, endpoint=self.endpoint, token='token1')

    def test_unique_dds_id(self):
        DDSUserCredential.objects.create(user=self.user1, endpoint=self.endpoint, dds_id='dds_id1')
        with self.assertRaises(IntegrityError):
            DDSUserCredential.objects.create(user=self.user1, endpoint=self.endpoint, dds_id='dds_id1')

    def test_unique_endpoint_and_user(self):
        DDSUserCredential.objects.create(endpoint=self.endpoint, user=self.user1, dds_id='dds_id1', token='token1')
        DDSUserCredential.objects.create(endpoint=self.endpoint, user=self.user2, dds_id='dds_id2', token='token2')
        with self.assertRaises(IntegrityError):
            DDSUserCredential.objects.create(endpoint=self.endpoint, user=self.user1, dds_id='dds_id3', token='token3')
